...
The scope is intentionally bigger, by implementing the pages after current Figma prototypes.
\subsection*{Deliverables}
The Sprint goal is to get the program finished, visual and functional for Scenario 1.
So the increment during this sprint is implementation of frontend of the overview page, optimizer page, production unit page and configuration page, Also implementation of a data manager in the backend. 
All that in the first half of the sprint. 
The second half is for connecting the frontend with the backend, so mostly data binding.
\subsection*{Sprint Planning}
This sprint is an experiment for how much work could be done in a sprint. 
So we decided to make a big leap by planning to implement most of the UI and the required connection to the backend. 
For the implementation for the connection is a data manager required that also needs to be implemented. 
...
We have some dependent work to do that also tests our group communication. 
For the open tickets of this sprint in the to do column (see Figure 3.3).
...
We had some communication issues that also contributed to the delay of some tickets, especially those in the in progress section. 
From timing perspective, it was possible to get all tickets done. 
...
\subsection*{Sprint Retrospective}
Because of underestimations and miss understanding by the scrum master, where there is no Retrospective written by each member.
Communication happened mostly in the Meetings, and there were rarely any impediments noticed.
Even if we decided to be more specific in the stand-ups of the meetings.
The members seem to be shy about their work and problems not saying anything in the meetings.
One has given an issue during their work, because of only implementing the frontend without the backend.
This ticket got delayed until the next sprint.

\subsection*{Reflections and Conflicts}
To resolve the communication issues we talked with each other and improved our Excalidraw board for writing down issues taking it more seriously, an option on Discord for main communiacation. 
The current scrum master will also get a task list of what he needs to do for a sprint. 
A member has now the options, for an issue, to publish early the branch what he/she is working on, writing down on discord or Excalidraw board, talking to a team member and also talking to the current scrum master. 
Also we made it clear that no one needs to be scared of each other, because of “bad code” or something.
